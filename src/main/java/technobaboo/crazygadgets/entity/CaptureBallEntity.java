...
public class CaptureBallEntity extends ThrownItemEntity {
	...
	protected void onEntityHit(EntityHitResult entityHitResult) {
		...
		ItemEntity itemEntity = new ItemEntity(entityHitResult.getEntity().getWorld(), hitPos.x, hitPos.y, hitPos.z,
				item);
	...
	protected void onCollision(HitResult hitResult) {
		...
		if (!this.getWorld().isClient() && hitResult.getType() != HitResult.Type.ENTITY) {
			...
			if (!ownerIsPlayer || (ownerIsPlayer && !playerOwner.isCreative())) {
				...
				ItemEntity itemEntity = new ItemEntity(getWorld(), hitPos.x, hitPos.y, hitPos.z, item);
				getWorld().spawnEntity(itemEntity);
			}
