...
import technobaboo.crazygadgets.effect.EnderPoof;
...
public class ChronoPearlEntity extends ThrownItemEntity {
	...
	protected void onEntityHit(EntityHitResult entityHitResult) {
		...
		if (entity instanceof LivingEntity && !(entity instanceof PlayerEntity)) {
			...
			this.noClip = true;
			EnderPoof.Poof(entity.getWorld(), entity.getPos());
			entity.discard();
	...
	public void tick() {
		if (!this.dataTracker.get(ENTITY_NBT).isEmpty()) {
			...
			if (timeout == 0) {
				Entity entity = EntityType.loadEntityWithPassengers(this.dataTracker.get(ENTITY_NBT), this.getWorld(),
						(entityx) -> {
				...
				EnderPoof.Poof(this.getWorld(), entity.getPos());
				this.discard();
				return;
			}
